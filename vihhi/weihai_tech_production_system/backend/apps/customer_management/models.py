...
class ContactInfoChange(models.Model):
    ...
    approval_instance = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='contact_info_changes',
        verbose_name='审批实例'
    )
    ...
    created_by = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
        related_name='created_contact_info_changes',
        verbose_name='创建人'
    )
    created_time = models.DateTimeField(default=timezone.now, verbose_name='创建时间')
    approved_time = models.DateTimeField(null=True, blank=True, verbose_name='审批通过时间')
    ...
    class Meta:
        ...
        ordering = ['-created_time']
        indexes = [
            models.Index(fields=['contact', 'approval_status']),
            models.Index(fields=['change_type']),
            models.Index(fields=['created_time']),
        ]
    
    def __str__(self):
        return f"{self.contact.name} - {self.get_change_type_display()} - {self.get_approval_status_display()}"
    
    def apply_changes(self):
        """应用变更到联系人"""
        if self.approval_status != 'approved':
            raise ValueError('只有已通过的变更申请才能应用')
        
        contact = self.contact
        for field_name, values in self.change_content.items():
            if hasattr(contact, field_name):
                setattr(contact, field_name, values.get('new'))
        
        contact.save()
        self.approved_time = timezone.now()
        self.save(update_fields=['approved_time'])
