...
使用方法：
    python manage.py send_monthly_plan_reminder
    
建议配置为定时任务（crontab）：
    # 每月28日9点执行
    0 9 28 * * cd /path/to/project && /path/to/venv/bin/python manage.py send_monthly_plan_reminder
    
    # 或使用Celery Beat
    @periodic_task(run_every=crontab(hour=9, minute=0, day_of_month=28))
    def send_monthly_plan_reminder_task():
        call_command('send_monthly_plan_reminder')
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'开始发送月度工作计划提醒（时间：{timezone.now()}）...')
        ...
        next_month = (now.replace(day=1) + timezone.timedelta(days=32)).replace(day=1)
        next_month_str = next_month.strftime('%Y年%m月')
        
        # 构建通知内容
        subject = f'【月度工作计划提醒】请及时编制{next_month_str}工作计划'
...
根据工作计划管理制度，请在{current_month}28日前完成{next_month_str}工作计划的编制工作。

请及时登录系统，进入"计划管理"模块，点击"新建工作计划"进行编制。
...
{now.strftime('%Y年%m月%d日')}
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="line-height: 1.8; color: #333;">
                <p>尊敬的同仁：</p>
                <p>您好！</p>
                <p>根据工作计划管理制度，请在<strong style="color: #EF4444;">{current_month}28日前</strong>完成<strong>{next_month_str}工作计划</strong>的编制工作。</p>
                <p>请及时登录系统，进入"计划管理"模块，点击"新建工作计划"进行编制。</p>
