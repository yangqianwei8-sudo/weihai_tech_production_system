...
每月1日上午9点：系统自动汇总员工上月目标更新进度、月度计划完成情况，生成每月简报，一键发送给员工及其上级。

使用方式：
- 通过cron或celery beat调用：python manage.py generate_monthly_summary
- 建议cron配置：0 9 1 * * python manage.py generate_monthly_summary
"""
from django.core.management.base import BaseCommand
from django.contrib.auth.models import User
from backend.apps.plan_management.services.summary_service import send_monthly_summary_to_user
import logging
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('生成月报任务'))
        self.stdout.write(self.style.SUCCESS('=' * 60))
        ...
        for user in users:
            try:
                if not dry_run:
                    result = send_monthly_summary_to_user(user)
                    if result:
                ...
                logger.error(f"处理用户 {user.username} 的月报失败：{str(e)}", exc_info=True)
