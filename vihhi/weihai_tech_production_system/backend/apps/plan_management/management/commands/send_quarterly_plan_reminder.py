...
使用方法：
    python manage.py send_quarterly_plan_reminder
    
建议配置为定时任务（crontab）：
    # 每个自然季度的最后一个月15日9点执行（3月、6月、9月、12月的15日）
    0 9 15 3,6,9,12 * cd /path/to/project && /path/to/venv/bin/python manage.py send_quarterly_plan_reminder
    
    # 或使用Celery Beat
    @periodic_task(run_every=crontab(hour=9, minute=0, day_of_month=15, month_of_year='3,6,9,12'))
    def send_quarterly_plan_reminder_task():
        call_command('send_quarterly_plan_reminder')
"""
...
class Command(BaseCommand):
    ...
    def get_next_quarter_info(self, now):
        """获取下一个季度的信息"""
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'开始发送季度工作计划提醒（时间：{timezone.now()}）...')
        ...
        # 获取下一个季度信息
        now = timezone.now()
        quarter_info = self.get_next_quarter_info(now)
        
        quarter_str = f"{quarter_info['year']}年{quarter_info['quarter_name']}（{quarter_info['start_date'].strftime('%Y年%m月%d日')} - {quarter_info['end_date'].strftime('%Y年%m月%d日')}）"
        
        # 构建通知内容
        subject = f'【季度工作计划提醒】请及时编制{quarter_info["year"]}年{quarter_info["quarter_name"]}工作计划'
...
根据工作计划管理制度，请在每个自然季度的最后一个月15日前完成下一个季度工作计划的编制工作。

本次提醒：请及时编制{quarter_info['year']}年{quarter_info['quarter_name']}（{quarter_info['start_date'].strftime('%Y年%m月%d日')} - {quarter_info['end_date'].strftime('%Y年%m月%d日')}）的工作计划。
...
{now.strftime('%Y年%m月%d日')}
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="line-height: 1.8; color: #333;">
                <p>尊敬的同仁：</p>
                <p>您好！</p>
                <p>根据工作计划管理制度，请在<strong style="color: #EF4444;">每个自然季度的最后一个月15日前</strong>完成下一个季度工作计划的编制工作。</p>
                <p>本次提醒：请及时编制<strong>{quarter_info['year']}年{quarter_info['quarter_name']}（{quarter_info['start_date'].strftime('%Y年%m月%d日')} - {quarter_info['end_date'].strftime('%Y年%m月%d日')}）</strong>的工作计划。</p>
                <p>请及时登录系统，进入"计划管理"模块，点击"新建工作计划"进行编制，<strong>计划周期请选择"季度计划"</strong>。</p>
